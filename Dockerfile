...
RUN echo "deb https://packages.cloud.google.com/apt gcsfuse-focal main" > /etc/apt/sources.list.d/gcsfuse.list
...
RUN chown -R develop:develop /home/develop/.config

RUN mkdir -p /data/gcp-bucket-mounts
RUN chown -R develop:develop /data/gcp-bucket-mounts
