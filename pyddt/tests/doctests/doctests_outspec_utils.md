...
Read the sample file that ships with the code:
    ...
    >>> data = pyddt.util.outspec.csv_file_to_data_dict_list("config/output_spec.csv")

Turn everything off:

    >>> data2 = pyddt.util.outspec.all_vars_off(data)

Write it to a file:

    >>> pyddt.util.outspec.write_data_to_csv(data2, "/tmp/test-outspec_utils.csv")

Now read in the file we just created and verify that there are no variables
enabled.

    >>> pyddt.util.outspec.cmdline_entry(['--summary','/tmp/test-outspec_utils.csv'])
...
Enable a single variable (AVLN, yearly, by layer) and write out again
    ...
    >>> data2 = pyddt.util.outspec.toggle_on_variable(data2, "AVLN", 'yearly layer')
    >>> pyddt.util.outspec.write_data_to_csv(data2, "/tmp/test-outspec_utils.csv")
...
Looking at the original file however, there should be only the default variable(s) enabled:
    ...
    >>> pyddt.util.outspec.cmdline_entry(['-s', 'config/output_spec.csv'])
                    Name                Units       Yearly      Monthly        Daily          PFT Compartments       Layers    Data Type     Description
...
Enable all yearly variables at the highest detail possible in the test file
    ...
    >>> pyddt.util.outspec.cmdline_entry(['--max-yearly','/tmp/test-outspec_utils.csv'])
    0

Enable all monthly variables at the highest detail possible in the test file

    >>> pyddt.util.outspec.cmdline_entry(['--max-monthly','/tmp/test-outspec_utils.csv'])
    0

Enable all variables at the highest resolution possible in the test file

    >>> pyddt.util.outspec.cmdline_entry(['--max-output','/tmp/test-outspec_utils.csv'])
...
Clear the test file and check that it is actually empty
    ...
    >>> pyddt.util.outspec.cmdline_entry(['--empty','/tmp/test-outspec_utils.csv'])
    0
    >>> pyddt.util.outspec.cmdline_entry(['--summary','/tmp/test-outspec_utils.csv'])
                    Name                Units       Yearly      Monthly        Daily          PFT Compartments       Layers    Data Type     Description
    0
...
Check available options for a single variable
    ...
    >>> pyddt.util.outspec.cmdline_entry(['--show-options','NPP','config/output_spec.csv'])
