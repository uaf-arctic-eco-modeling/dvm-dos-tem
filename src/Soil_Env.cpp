...
void Soil_Env::updateDailySurfFlux(Layer *toplayer, const double &dayl)
{
  ...
  double insw = ed->d_v2g.swthfl * cd->m_vegd.fpcsum + ed->d_a2l.nirr * (1. - cd->m_vegd.fpcsum);
...
void Soil_Env::updateDailySM(double weighted_veg_tran)
{
  ...
  Layer *currl = fstsoill;
  if (drainl != NULL && ground->frnttype[0] != 1)
  { // no infil if top of ground frozen
  ...
  if (cd->drainage_type == 1)
  { // 0: well-drained; 1: poorly-drained
    baseflow = 0.0;
  }
...
double Soil_Env::getEvaporation(const double &dayl, const double &rad)
{
  ...
  if (daylsec < 0.1)
  { // 0.1 sec for mathematical purpose,
...
{
  double wtd = 0.0;
  if (ground->ststate == 1 || ground->fstsoill->frozen == 1)
  { // soil column is frozen, so no water table
    return wtd;
  }
  ...
  while (currl != NULL && currl->isSoil)
  {
    ...
    if (currl == ground->fstfntl)
    {                                              // if top thaw front is in this layer, only use the dz above it
    ...
    else
    {
      ...
      if (currl->nextl != NULL && currl->frozen == -1 && !currl->nextl->isRock)
      {
        ...
        double nextl_dz_unfrozen = currl->nextl->dz * (1 - currl->nextl->frozenfrac);
      ...
      { // current layer is partly frozen or next layer is null or rock
      ...
      if (wtd > ground->lstsoill->z + ground->lstsoill->dz)
      { // wtd is in rock layer
...
void Soil_Env::retrieveDailyTM(Layer *toplayer, Layer *lstsoill)
{
  ...
  while (curr2 != NULL)
  {
    if (curr2->isSoil)
    {
      ...
      {
        ed->d_soid.aws[soilind] = curr2->getVolLiq() / (curr2->poro - curr2->getVolIce()); // FIX THIS: divide by zero when poro == getVolIce()!
      }
