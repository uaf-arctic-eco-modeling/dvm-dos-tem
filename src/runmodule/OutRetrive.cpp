...
//the following is needed, because daily output is done in the last
//  day of the month, and then data storing is the best choice
void OutRetrive::assignSiteDlyOutputBuffer_Env(snwstate_dim d_snow,
                                               const int &ipft,
                                               const int &iday) {
  if (ipft<0) {
...
void OutRetrive::updateRegnOutputBuffer(const int & im) {
  ...
  for (int ip=0; ip<NUM_PFT; ip++) {
    ...
    if (im==11 && regnod->outvarlist[I_nuptake]==1) {
      regnod->nuptake[0][ip] = bd[ip]->y_soi2v.lnuptake
                               + bd[ip]->y_soi2v.snuptakeall;
    } else if (regnod->outvarlist[I_nuptake]==2) {
      regnod->nuptake[im][ip] = bd[ip]->m_soi2v.lnuptake
  ...
  if (im==11 && regnod->outvarlist[I_rh]==1) {
    regnod->rh[0] = bdall->y_soi2a.rhrawcsum //note: 'bd' soil portion MUST
                                             //  BE exactly SAME for all PFTs
...
///////////////////////
  ...
  if (im==11 && regnod->outvarlist[I_burnretainc]==1) {
    //retained abvc is burned residue, retained blwc is burn-caused root death
    regnod->burnretainc[0] = fd->fire_v2soi.abvc+fd->fire_v2soi.blwc;
...
// NOTE: 'resod', restartoutput data, a dataset to resume a complete model
//         run if model is paused. This is useful and very needed for carrying
//         out a series of model implementation, i.e., from
//         eq->spinup->transient->scenario  runs OR, potentially the model
//         can run spatially for one time-step (rather than in time
//         series for ONE cohort)
void OutRetrive::updateRestartOutputBuffer() {
  ...
  //this is 'fire_a2soi.orgn' to re-deposit fire-emitted N in one FRI
  resod->firea2sorgn = fd->fire_a2soi.orgn;
  //vegegetation
  ...
  for (int ip=0; ip<NUM_PFT; ip++) {
    if (cd->m_veg.vegcov[ip]>0.) {
      ...
      resod->foliagemx[ip] = cd->m_vegd.foliagemx[ip];
  ...
  for(int il =0; il<cd->d_snow.numsnwl; il++) {
    ...
    resod->TSsnow[il]  = edall->d_snws.tsnw[il];
