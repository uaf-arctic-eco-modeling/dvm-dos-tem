...
*         and 4 soil C pools (raw materials, active SOM,
*                            physically-resistant SOM, chemically-resistant SOM)
*         1 woody debris C pools 2 soil N pools (organic N, available N)
*
*        2)vegetation, soil are integrated separately, because vegetation
*          may have multple PFTs, but all of which share ONE soil. Here,
*          the code only deals with ONE PFT and/or ONE SOIL, by setting
*          the following switches:
*            bool vbgc, bool sbgc
*
*          And, the 'bd' (BgcData) for veg and soil may not be same
*            - be cautious here!
*
*/
...
Integrator::Integrator() {
  ...
  for(int i =0; i<NUM_PFT_PART; i++) { //Yuan: here is the reason that the "temconst.h" is needed
    ...
    strcpy(predstr_veg[I_VEGC+i],(string("VEGC") +str1).c_str());
    // vegetation nitrogen pools
    strcpy(predstr_veg[I_STRN+i],(string("STRN") +str1).c_str());
  }
  ...
  for(int i =0; i<NUM_PFT_PART; i++) { //Yuan: here is the reason that the "temconst.h" is needed
    ...
    strcpy(predstr_veg[I_INGPP+i],(string("INGPP") +str2).c_str());
    // NPP not limited by nutrient availability
    strcpy(predstr_veg[I_INNPP+i],(string("INNPP") +str2).c_str());
    ...
    strcpy(predstr_veg[I_RM+i],(string("RM") +str2).c_str());
    // plant growth respiration
    strcpy(predstr_veg[I_RG+i],(string("RG") +str2).c_str());
    //litterfall carbon
    strcpy(predstr_veg[I_LTRC+i],(string("LTRC") +str2).c_str());
    // vegetation nitrogen uptake for structural components
    strcpy(predstr_veg[I_SNUP+i],(string("SNUP") +str2).c_str());
    // nitrogen mobilization by vegetation
    strcpy(predstr_veg[I_NMBOL+i],(string("NMBOL") +str2).c_str());
    // nitrogen resorption by vegetation
    strcpy(predstr_veg[I_NRSRB+i],(string("NRSRB") +str2).c_str());
    // litterfall nitrogen
    strcpy(predstr_veg[I_LTRN+i],(string("LTRN") +str2).c_str());
  }

  strcpy(predstr_veg[I_INNUP],"INNUP"); //vegetation N uptake at full capacity
                                        //  without interaction from C
  strcpy(predstr_veg[I_LNUP],"LNUP" ); //vegetation nitrogen uptake for
                                       //  labile components
  ...
  for(int il =0; il<MAX_SOI_LAY; il++) { //Yuan: here is the reason that the "temconst.h" is needed
    ...
    //soil raw material carbon
    strcpy(predstr_soi[I_L_RAWC+il],(string("RAWC") +str2).c_str() );
    // soil active som carbon
    strcpy(predstr_soi[I_L_SOMA+il],(string("SOMA") +str2).c_str() );
    // soil physically-resistant som carbon
    strcpy(predstr_soi[I_L_SOMPR+il],(string("SOMPR") +str2).c_str() );
    // soil chemically-resistant som carbon
    strcpy(predstr_soi[I_L_SOMCR+il],(string("SOMCR") +str2).c_str() );
  }
  ...
  for(int il =0; il<MAX_SOI_LAY; il++) { //Yuan: here is the reason that the "temconst.h" is needed
    ...
    string str2 = ilayer.str();
    //soil rh
    strcpy(predstr_soi[I_L_RH_RAW+il],(string("RHRAW") +str2).c_str());
    strcpy(predstr_soi[I_L_RH_SOMA+il],(string("RHSOMA") +str2).c_str());
    strcpy(predstr_soi[I_L_RH_SOMPR+il],(string("RHSOMPR") +str2).c_str() );
    strcpy(predstr_soi[I_L_RH_SOMCR+il],(string("RHSOMCR") +str2).c_str() );
    // soil organic nitrogen
    strcpy(predstr_soi[I_L_ORGN+il],(string("ORGN") +str2).c_str() );
    // soil available nitrogen
    strcpy(predstr_soi[I_L_AVLN+il],(string("AVLN") +str2).c_str() );
    // soil net N minerization
    strcpy(predstr_soi[I_L_NMIN+il],(string("NMIN") +str2).c_str() );
    strcpy(predstr_soi[I_L_NIMMOB+il],(string("NIMMOB") +str2).c_str() );
  }
...
//       if (err[I_VEGC+i] > fabs( ptol * ptstate[I_VEGC+i] ) ){ // this may have issues if the err is very small while ptstate is 0
    ...
    if (same>zero) {
      return test = vegvarkey(I_DEADC)+1;
    }
  ...
  if (soibgc) {
    ...
    for(int il =0; il<numsl; il++) {
      ...
      if (same>zero) {
        return test = soivarkey(I_L_SOMPR) +1 + il;
      }

      same = err[I_L_SOMCR+il] - fabs( ptol * ptstate[I_L_SOMCR+il]);

      if (same>zero) {
        return test = soivarkey(I_L_SOMCR)+1 + il;
      }
    ...
    if (same>zero) {
      return test = soivarkey(I_WDEBRISC)+1;
    }
    ...
    if (ssl->get_nfeed()) {
      for(int il =0; il<numsl; il++) {
        ...
        if (same>zero) {
          return test = soivarkey(I_L_ORGN)+1+il;
        }

        same = err[I_L_AVLN+il] - fabs(ptol * ptstate[I_L_AVLN+il]);

        if (same>zero) {
          return test = soivarkey(I_L_AVLN)+1+il;
        }
        ...
        if (same>zero) {
          return test = soivarkey(I_L_NIMMOB)+1+il;
        }
