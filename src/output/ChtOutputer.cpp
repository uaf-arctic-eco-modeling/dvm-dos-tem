...
void ChtOutputer::init(string & dirfile) {
  ...
  //'snowextramass' refers to the snow mass which not enough for a a
  //  now layer (partially covered)
  snwextramassV  = ncfile->add_var("SNWEXTRAMASS", ncDouble, timeD);
...
void ChtOutputer::outputCohortDimVars_mly(CohortData *dimod,
                                          const int & tstepcnt) {
...
void ChtOutputer::outputCohortDimVars_yly(CohortData *dimod,
                                          const int & tstepcnt) {
  NcError err(NcError::verbose_nonfatal);
