...
void EnvOutputer::outputCohortEnvVars_dly(const int &ipft, EnvDataDly * envod,
                                          const int&iy, const int&im,
                                          const int &id, const int & tstepcnt) {
  NcError err(NcError::verbose_nonfatal);

  if (ipft<=0) {
    if (tstepcnt==0) {
      chtidV->put(&envod->chtid);
    }
...
void EnvOutputer::outputCohortEnvVars_mly(const int &ipft, snwstate_dim *m_snow,
                                          EnvData* envod, const int&iy,
                                          const int&im, const int & tstepcnt) {
  NcError err(NcError::verbose_nonfatal);

  if (ipft<0) {
    if (tstepcnt==0) {
      chtidV->put(&envod->cd->chtid);
    }
...
void EnvOutputer::outputCohortEnvVars_yly(const int &ipft, snwstate_dim* y_snow,
                                          EnvData *envod, const int&iy,
                                          const int & tstepcnt) {
  NcError err(NcError::verbose_nonfatal);
