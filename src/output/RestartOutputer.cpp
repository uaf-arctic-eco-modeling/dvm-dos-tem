...
void RestartOutputer::init(string& outputdir,string& stage) {
  ...
  //fire-emitted N deposition
  firea2sorgnV = restartFile->add_var("FIREA2SORGN", ncInt, chtD);
  // - veg
  ...
  eetmxAV        =restartFile->add_var("EETMXA", ncDouble, chtD,
                                       prvyearD, pftD);
  growingttimeAV =restartFile->add_var("GROWINGTTIMEA", ncDouble,
                                       chtD, prvyearD, pftD);
  unnormleafmxAV =restartFile->add_var("UNNORMLEAFMXA", ncDouble,
                                       chtD, prvyearD, pftD);
  //snow
  ...
  FROZENFRACsoilV=restartFile->add_var("FROZENFRACsoil", ncDouble,
                                       chtD, soillayerD);
  ...
  prvltrfcnAV  =restartFile->add_var("PRVLTRFCNA", ncDouble, chtD,
                                     prvmonthD, soillayerD);
}
