...
//                   "unsupported type for template argument T!" );
  ...
  */
  ...
  std::array<double, MAX_SOI_LAY> read_soil_var_notime(const std::string &filename, const std::string &varname, const int y, const int x);
  ...
  std::array<double, NUM_PFT> read_pft_var_notime(const std::string &filename, const std::string &varname, const int y, const int x);
  ...
  std::array<std::array<double, NUM_PFT>, NUM_PFT_PART> read_veg_var_notime(const std::string &filename, const std::string &varname, const int y, const int x);
  ...
  template <typename PTYPE>
  void output_nc_3dim_notime(OutputSpec* out_spec, std::string stage_suffix,
                             cell_coords* coords, PTYPE data,
                             int max_var_count){
    BOOST_LOG_SEV(glg, debug)<<"output_nc_3dim_notime, var: "<<out_spec->var_name;
    //timestep, row, col
    size_t datastart[3];
    datastart[0] = 0;
    datastart[1] = coords->yidx;
  ...
  template<typename PTYPE>
  void output_nc_4dim_notime(OutputSpec* out_spec, std::string stage_suffix,
                             cell_coords* coords, PTYPE data,
                              int max_var_count_1, int max_var_count_2){

    BOOST_LOG_SEV(glg, debug)<<"output_nc_4dim_notime, var: "<<out_spec->var_name;
    //compartment, pft, row, col
    size_t datastart[4];
    datastart[0] = 0;
    datastart[1] = 0;
    datastart[2] = coords->yidx;
    ...
    size_t datacount[4];
    datacount[0] = max_var_count_1;
    datacount[1] = max_var_count_2;
    datacount[2] = 1;
    datacount[3] = 1;
  ...
  void output_nc_5dim(OutputSpec* out_spec, std::string stage_suffix,
    ...
    datastart[1] = 0;
    datastart[2] = 0;
    datastart[3] = coords->yidx;
    datastart[4] = coords->xidx;
