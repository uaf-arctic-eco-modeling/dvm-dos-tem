...
def run_TEM(x):
    ...
    return dvmdostem.get_calibration_outputs()[8:22]

def get_param_targets():
    return dvmdostem.get_calibration_outputs(calib=True)[8:22]

dvmdostem=TEM.TEM_model()
dvmdostem.site='/data/input-catalog/cru-ts40_ar5_rcp85_mri-cgcm3_MurphyDome_10x10'
dvmdostem.work_dir='/data/workflows/MD1-STEP4-CR'
...
md = Mads.createproblem(initial_guess, obs, TEM_pycall;
    ...
    paramlog=[falses(4); falses(10); trues(10)],
    problemname="STEP4-MD1-CR")
...
Mads.spaghettiplot(md, forward_predictions,
                       xtitle="# of observations", ytitle="NPP(namx),VEGC(krb,cfall)",filename="STEP4-MD1-CR-matchplot.png")
