...
**Step 2** continues in [`calibration_instructions.md`](../2_calibration/calibration_instructions.md#step-1-handoff).

Path conventions: see [Multisite conventions](../0_setup/setup_instructions.md#multisite-conventions). Workflow root is `/data/workflows/CMT{cmtnum:02d}-{site_label}/`.

Reference notebook (interactive only): [`calibration_process.ipynb`](../notebooks/calibration_process.ipynb) — bundled [`sa-step1-example-imn.yaml`](sa-step1-example-imn.yaml) is one CMT04 example; agent runs use `logs/sa-{site_label}-*.yaml` from the template.

## Directory layout

| Location | Tracked | Agent use |
|----------|---------|-----------|
| `agent/1_cmax/` (this folder) | Yes | Templates and scripts |
...
Provide these inputs from the Phase 0 setup manifest:
...
Example (CMT04 Imnavait): `cmtnum: 4`, `site: /data/input-catalog/Imnavait`, `site_label: IMN`.
...
- Phase 0 complete — [`setup_instructions.md`](../0_setup/setup_instructions.md); `seed_path` and `site` from `logs/{site_label}-setup-manifest.yaml`
- `dvmdostem-autocal` running; `/work`, `/data/input-catalog`, `/data/workflows` mounted
- CMT has `GPPAllIgnoringNitrogen` in `calibration/calibration_targets.py`
- Active PFTs identified for `{cmtnum}` in `cmt_bgcvegetation.txt` at `seed_path`
- **Do not** `pip install` in the container
...
| Headless analysis CLI | `/work/mads_calibration/agent/1_cmax/step1_analyze.py` | Preferred over inline Python for analysis phase. |
| Recovery seed setup | `/work/mads_calibration/agent/1_cmax/step1_recovery_setup.py` | Perturbed seed dirs for runs A–D. |
| Yaml template | `/work/mads_calibration/agent/1_cmax/sa-step1-template.yaml` | Parameterized Step 1 config template. |
| Recovery manifest template | `/work/mads_calibration/agent/1_cmax/recovery_cmax_optima.yaml` | Bias tiers and run definitions for perturbation loop. |
...
cmtnum = {cmtnum}  # replace with input
...
- **One SA per VM:** do not start Step 1 while a Step 2 SA (or another Step 1) is still running in the same `dvmdostem-autocal` container — see [SA concurrency](../0_setup/setup_instructions.md#sa-concurrency-per-vm).
...
**Step 2 handoff:** `seed_setup.py` refuses `best_effort` unless `--force` (see [`calibration_instructions.md`](../2_calibration/calibration_instructions.md#step-1-handoff)).
...
For sites other than CMT04, scale `bias_tiers` per-PFT to match local `cmax` magnitudes before running setup.

```bash
docker compose exec -T dvmdostem-autocal bash -c \
  'python /work/mads_calibration/agent/1_cmax/step1_recovery_setup.py \
    --manifest /work/mads_calibration/agent/1_cmax/recovery_cmax_optima.yaml \
    --reference-cmax-yaml /data/workflows/CMT{cmtnum:02d}-{site_label}-sa-N{N}/step1-result.yaml \
    --write-manifest /data/workflows/CMT{cmtnum:02d}-{site_label}/recovery-manifest.yaml \
    --cmtnum {cmtnum} \
    --dest-base /data/workflows/CMT{cmtnum:02d}-{site_label} \
    --runs A B C D'
```

`dest-base` is always `/data/workflows/CMT{cmtnum:02d}-{site_label}`.
...
recommended_cmax:
  cmax_pft0: 243.20
  # ... one key per active PFT for this CMT
```
...
| Notebook Step 1 example (CMT04 Imnavait) | `mads_calibration/agent/1_cmax/sa-step1-example-imn.yaml` |
